...
class Neuron:
    ...
    def __init__(self, num, V_T = 1, lam = 1, t_int = 10):
        ...
        self.t_int = t_int   # internal time steps for rk4 (within spike -- int>0)
    ...
    def time_step(self, spikes_in, w_in, feedb_in = None, w_r = None, rec = False):
        ...
        Implements 1st-order Euler step solution of LIF neuron dynamics.
        ...
        rec       = bool depending on if neuron has recurrent connections
        ...
        self.V = (self.V * (self.V > 0)) - (self.V * (self.V >= self.V_T))
    ...
    def clear_spikes(self, num = 0):
        """
        Clears internal memory of spikes. Allows user to select how far back 
        to clear spikes
        """
        if num < 1:
            self.spikes_out = []
...
class Layer:
    ...
    def infer(self, batch_inp, start_idx, stop_idx, t_d, reuse=True):
        ...
        if self.rec:
            curr_t = len(self.neur_list[0].spikes_out)
            if (start_idx == 0) and (curr_t == 0):
    ...
    def max_V(self):
        # get maximum max_V from list of neurons
        max_max_V = 0
        for neur in self.neur_list:
            max_max_V = np.max([max_max_V, neur.V_max])
        return max_max_V

    def update_V_T(self, V_in):
        # update threshold of all neurons in list
        for neur in self.neur_list:
            neur.V_T = V_in

    def clear_neurs(self):
        # clear all neuron spikes/voltages
        for neur in self.neur_list:
            neur.clear_spikes()
            neur.clear_V(clear_V_mem = True)
...
class SNN:
    ...
    def thresh_bal(self, Vs):
        """
        Updates thresholds with maximum activations found running training dataset through ANN.
        Input should be list of activations from each layer.
        Ex/ single layer means input should be in form: [float]
        """
        for i in range(len(self.layers)):
            self.layers[i].update_V_T(Vs[i])

    def infer(self, spikes_in, t_b, t_d, reuse=True):
        """
        Performs simulation of connected, spiking neurons with variable binning and delay.

        Inputs:
        spikes_in   = list of spikes input to first layer (size: (size of prev. layer, any length))
                      Note: if second dimension not multiple of batch size, remainder will be unused
        t_b         = number of steps in each temporal batch to be computed before passing 
        ...
        Outputs:
        activation  = numpy array of integers of size: [last layer dimension]
